...
, то нам нужна команда - **cherry-pick**. Она позволит нам перенести сразу бесконечное множество *commits* в место где мы сейчас находимся (при правильном использвоний поможет сократить время на добавление *commits* в основную ветку).
```sh
